...
def get_blog_list_common_date(request, blogs_all_list):
    ...
    paginator = Paginator(blogs_all_list, each_page_blogs_number)  # 每10页进行分页
    page_num = request.GET.get('page', 1)  # 获取页码参数get请求,后面的1是默认
    page_of_blogs = paginator.get_page(page_num)  # 具体页码的页面
    # 优化页码范围
    page_num = page_of_blogs.number  # 获取当前页码
    ...
    if paginator.num_pages - page_range[-1] >= 2:
        page_range.append('...')
    if page_range[0] != 1:
        page_range.insert(0, 1)  # 加入第一页
    if page_range[-1] != paginator.num_pages:
        page_range.append(paginator.num_pages)  # 加入最后一页
    ...
    context = {}
    context['blog_types'] = blog_type_list  # 导航栏列表博客类型
    context['page_of_blogs'] = page_of_blogs  # 分页博客
    context['page_range'] = page_range  # 页码展示
...
def blog_detail(request, pk):
    ...
    comment=Comment.objects.filter(content_type=blog_content_type,object_id=pk)
...
#     return render(request, 'blog/../templates/blogger/login.html', context)
#
...
#     return render(request, 'blog/../templates/blogger/register.html', context)
